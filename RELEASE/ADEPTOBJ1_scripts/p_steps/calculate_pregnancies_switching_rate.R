...
for (trt in seq_along(common_keys)) {
  ...
  if (!deap_flags$is_EFEMERIS && !deap_flags$is_FIN_REG) dt <- merge(dt_prepreg[, .(person_id, pregnancy_id)], dt_switch, by = c("person_id"), all = FALSE)
  if (deap_flags$is_EFEMERIS || deap_flags$is_FIN_REG)   dt <- merge(dt_prepreg[, .(pregnancy_id)], dt_switch, by = c("pregnancy_id"), all = FALSE)
