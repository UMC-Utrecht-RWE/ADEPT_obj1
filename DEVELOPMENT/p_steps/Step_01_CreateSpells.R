...
# Load observation periods file
OBSERVATION_PERIODS <- IMPORT_PATTERN(pat = "OBSERVATION_PERIODS", dir = path_CDM_dir)  
...
if (SUBP) {
  ...
  for (i in 1:nrow(subpopulation_meanings)) {
    ...
    if (nrow(TEMP) > 0) {
      ...
      if (length(strsplit(subpopulation_meanings[["subpopulations"]][i], "_")[[1]]) > 1) {
        ...
        while (k < length(meaning_sets)) { 
          ...
          # Rename column for start date in overlap
          setnames(overlap, "entry_spell_category", "op_start_date")
          
          # Rename column for end date in overlap
    ...
    # Save TEMP 
    saveRDS(TEMP, file = paste0(path_study_source_pop, "/", subpopulation_meanings[["subpopulations"]][i], "_OBS_SPELLS.rds"))
    ...
    saveRDS(FlowChartCreateSpells, file = paste0(path_g_output, "/", subpopulation_meanings[["subpopulations"]][i], "_flowchart.rds"))
  ...
  # Keep the last spell per person
  ...
  # Save Obervation Periods
  saveRDS(OBSERVATION_PERIODS1, file = paste0(path_study_source_pop, "/ALL_OBS_SPELLS.rds"))
  ...
  # Corresponding numbers for each step in the flowchart:
  # 'before_CreateSpells' = original total rows
  # 'step0_unique' presumably holds count of unique person IDs before processing (must be defined earlier)
  # 'after_CreateSpells' = count after merging spells with gap allowed
  # 'select_most_recent' = count after selecting most recent spell per person
  ...
  # Save the flowchart
  saveRDS(FlowChartCreateSpells, file = paste0(path_g_output, "/FlowChartCreateSpells.rds"))
  
  # If an object called FlowChartOverlap exists, Save it
  if (exists("FlowChartOverlap")) {
    
    saveRDS(FlowChartOverlap, file = paste0(path_study_source_pop_tmp, "/SUBPOP_FlowChartOverlap.rds"))
