...
for(i in 1:nrow(SCHEME_05)){
  ...
  SOURCE_POPULATION <- SOURCE_POPULATION[, PY_OP := round((op_end_date - op_start_date) / 365.25, 2)][,year_op_start := year(op_start_date)]
...
saveRDS(SCHEME_05, file = file.path(paths$D5_dir, "flowcharts", "scheme_05.rds"))
