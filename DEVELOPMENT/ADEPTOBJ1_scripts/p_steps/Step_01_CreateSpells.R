...
# and in the case of multiple spells, the most recent is taken,
# and the others are discarded, resulting in one spell per personID
...
if (SUBP) {
  ...
  for (i in seq_len(nrow(op_meaning_list_set))) {
  ...
  for (i in seq_len(nrow(subpopulation_meanings))) {
      ...
      if (length(strsplit(subpopulation_meanings[["subpopulations"]][i], "_")[[1]]) > 1) {
        ...
        while (k < length(meaning_sets)) {
          ...
          if (k < length(meaning_sets) - 1) {
            ...
            TEMP <- rbind(overlap, TEMP, fill = TRUE)
