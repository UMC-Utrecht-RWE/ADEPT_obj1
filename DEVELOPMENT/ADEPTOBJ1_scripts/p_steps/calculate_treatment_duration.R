...
# If pop_prefix is PC, then drop any that are PC_HOSP
if(pop_prefix == "PC") {
  files_episodes <- files_episodes[!grepl("PC_HOSP", files_episodes)]
}
...
# Save combined summary
saveRDS(final_summary, file.path(paths$D5_dir, "1.2_treatment_duration", paste0(pop_prefix, "_all_treatment_duration_summary_groups.rds")))
