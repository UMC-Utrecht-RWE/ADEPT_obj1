...
if(!deap_flags$is_EFEMERIS){
  files_polytherapy_episodes <- files_polytherapy_episodes[grepl(paste0("^", pop_prefix, "_"), files_polytherapy_episodes)] # pop prefix
  if(pop_prefix=="PC") files_polytherapy_episodes <- files_polytherapy_episodes[!grepl("PC_HOSP", files_polytherapy_episodes)] #BIFAP
  
  files_indication <- files_indication[grepl("_F_", files_indication)] # Females only 
  if (pop_prefix == "PC") files_indication <- files_indication[!grepl("PC_HOSP", basename(files_indication))] # BIFAP
}

if(length(files_polytherapy_episodes)>0){
  # read and bind files 
  ...
  if(!deap_flags$is_EFEMERIS){
    ...
    # Keep necessary columns 
    dt_indication <- dt_indication[, .(person_id, pregnancy_id, event_date, event_definition)] 
  ...
  message(red("No polytherapy episodes found for stratification"))
