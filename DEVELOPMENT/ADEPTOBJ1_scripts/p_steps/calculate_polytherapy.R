...
if (!deap_flags$is_EFEMERIS && !deap_flags$is_FIN_REG) {
  files_episodes <- files_episodes[grepl(paste0("^", pop_prefix, "_"), files_episodes)] # gets the female files 
  denominator <- readRDS(file.path(paths$D3_dir, "denominator", paste0(pop_prefix, "_denominator.rds"))) # denominator file 
}
...
for (epi1 in seq_along(files_episodes)) {
  ...
  for (epi2 in seq_along(files_episodes)) {
    ...
    if (!deap_flags$is_EFEMERIS && !deap_flags$is_FIN_REG) {
      ...
      # Subset data to contain id's common to both data sets
      ...
      overlaps <- foverlaps(
        x = dt1_sub,
        y = dt2_sub,
        by.x = c("person_id", "window_start", "window_end"),
      ...
      overlaps <- foverlaps(
        x = dt1_sub,
        y = dt2_sub,
        by.x = c("pregnancy_id", "window_start", "window_end"),
        by.y = c("pregnancy_id", "window_start", "window_end"),
        type = "any",
        nomatch = 0L
      )
      ...
      # We only need this for obj 1.4, where there needs to be an overlap of 3 months with pregnancy
